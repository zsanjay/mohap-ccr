 ...
 */
export class BranchOverviewDataSource extends DataSource<AvgWaitingTimeBranch> {
  ...
  this.dataProvider.getBranchOverviewData()
  .subscribe(res => { 
    this.data.next(res);
    ...
    let avgTrtTime = this.utilityService.getAverageTime(temp.avgTrtTime);
    temp.avgTrtTime = avgTrtTime;
  ...
  filterByRegion(branchIds : number[]): any {

  this.dataProvider.findBranchOverviewDataByBranchIds(branchIds)
  .subscribe(res => {
    this.data.next(res);
