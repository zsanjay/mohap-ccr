...
export class QueueDataSource extends DataSource<AvgWaitingTimeBranch> {
  ...
  this.dataProvider.getTransQueuesByBranchId(value , branchId).pipe(
    catchError(() => of([])),
   ...
   getSortedData(sort : Sort) {
    ...
    this.sortedData.sort((a, b) => {
      ...
      switch (this.sort.active) {
        ...
        return ( (a.avgTrtTime > 0 && b.avgTrtTime > 0  && a.avgTrtTime < b.avgTrtTime) ? -1 : 1) * (isAsc ? 1 : -1);
  ...
  this.dataProvider.getTransQueuesByBranchId(filtercriteria.sort , filtercriteria.branchId).pipe(
    catchError(() => of([])),
    finalize(() => this.loadingData.next(false))
  )
  .subscribe(res => { 
